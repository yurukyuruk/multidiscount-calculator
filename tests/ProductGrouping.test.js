...
describe(ProductGrouping.name, () => {
  ...
  test('check if returns expected object 1', () => {
    ...
    expect(validator.useInputData(array1, array2)).toEqual([
      { names: ['a', 'b', 'c'], propotionalDiscount: 150 },
      { names: ['d', 'e'], propotionalDiscount: 0.1 },
    ]);
  });
  ...
  test('check if returns expected object 2', () => {
    ...
    expect(validator.useInputData(array1, array2)).toEqual([{ names: ['a', 'b', 'c'], propotionalDiscount: 150 }]);
  });
