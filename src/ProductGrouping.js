  ...
  useInputData(discountInputValues, productInputValues) {
    ...
    discountDefinitionSortedFromLowestItem.forEach((discountDefinitionRow) => {
      ...
      for (let i = 0; i < productsInformationSortedFromHighestPrice.length; i++) {
        let eachProductDiscountAmount =
          productsInformationSortedFromHighestPrice[i][1] * discountDefinitionRow[1] * 0.01;
        productsWithDiscountAmountsDependingOnItemAmount.push([
          productsInformationSortedFromHighestPrice[i][0],
          eachProductDiscountAmount,
        ]);
      }
      ...
      for (let j = 0; j < productsWithDiscountAmountsDependingOnAllItemAmounts[i].length; j++) {
        ...
        for (let k = j + 1; k < j + Number(discountDefinitionSortedFromLowestItem[i][0]); k++) {
          if (productsWithDiscountAmountsDependingOnAllItemAmounts[i][k]) {
            productsAndTheirProportionalDiscount.names.push(
              productsWithDiscountAmountsDependingOnAllItemAmounts[i][k][0]
            );
        ...
        if (
          productsAndTheirProportionalDiscount.names.length === Number(discountDefinitionSortedFromLowestItem[i][0])
        ) {
    ...
    let sortedProductsAndTheirProportionalDiscountForAnItemCount =
      productsAndTheirProportionalDiscountForAnItemCount.sort((a, b) => {
    ...
    let i = 0;
    if (
      productsNamesSortedFromHighestPrice.length -
        sortedProductsAndTheirProportionalDiscountForAnItemCount[0].names.length >=
      Number(discountDefinitionSortedFromLowestItem[0][0])
    ) {
      do {
        ...
        iterated = filterObjectsByCondition(
          sortedProductsAndTheirProportionalDiscountForAnItemCount,
        ...
        i < productsNamesSortedFromHighestPrice.length &&
        productsNamesSortedFromHighestPrice.length - i >= Number(discountDefinitionSortedFromLowestItem[0][0])
      );
    }
